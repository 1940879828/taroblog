...
// 生成每一页的 key（URL）。第 1 页的 key 已由服务端预填到缓存，SWR 不会真正请求它。
const getKey = (pageIndex: number) =>
  `/api/waterfall-items?page=${pageIndex + 1}`
...
export default function SwrWaterfallClient({
  ...
  // useSWRInfinite：按 getKey 逐页加载。第一页数据已由服务端通过 fallback 预填，不重复请求。
  const { data, size, setSize, isLoading } = useSWRInfinite<PageResult>(
    getKey,
    fetcher
  )

  // 把所有页的 items 摊平（第一页来自服务端 fallback，后续页来自 SWR 请求）
  ...
  return (
    // 用 SWRConfig fallback 把"服务端取好的第一页"预填进第 1 页 key 的缓存，
    // 这样 SWR 读到第 1 页时直接用，不发网络请求 —— 这就是"服务端首屏 + 客户端增量"。
    <SWRConfig value={{ fallback: { [getKey(0)]: firstPage } }}>
      <div className="rounded-xl border border-slate-700 bg-slate-950 p-4">
        <p className="mb-3 text-xs text-emerald-300">
          客户端组件 —— 已加载 {items.length} 张卡片（第 {size}{" "}
          页，第一页来自服务端）
        </p>
